...
if __name__ == '__main__':
	...
	white_y_train.replace(2, -1, inplace=True)
	white_y_test.replace(2, -1, inplace=True)
	...
	EST = LabelSpreading('knn', max_iter=2, n_neighbors=3, tol=0.01, n_jobs=-1)
	EST.fit(white_X_train, white_y_train)
	white_y_pred = EST.predict(white_X_test)
	score = f1_score(white_y_test, white_y_pred, average='macro')
	print('f1Macro', score)
	...
	## save predicitons
	y_pred = le.inverse_transform(white_y_pred).astype(str)
